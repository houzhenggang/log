...
//    snprintf(key, sizeof(key), CONST_RUNTIME_IS_AUTOLOGOUT_ENABLED);/*logout detele*/
...
//    if(ntop->getPrefs()->do_auto_logout() && strncmp(buf, (char*)"1", 1) == 0) {/*logout detele*/
//        snprintf(key, sizeof(key), "sessions.%s", session_id);
...
//    callbacks.begin_request = handle_lua_request;//处理后台（Web server ）请求
