...
GenericHash::GenericHash(NetworkInterface *_iface, u_int _num_hashes, u_int _max_hash_size) {
    ...
    purge_step = max_val(num_hashes / PURGE_FRACTION, 1);//1024
...
/* ************************************ */
/*hash条目的添加(flow host(src,dst) mac vlan ases )*/
bool GenericHash::add(GenericHashEntry *h) {
    if(hasEmptyRoom()) {//检查hash是否有空间,超过最大值添加失败
...
u_int GenericHash::purgeIdle() {
    ...
    for(u_int j = 0; j < purge_step; j++) {//purge_step=1024?
        ...
        if(table[i] != NULL) {
            ...
            while(head) {
                ...
                if(head->is_ready_to_be_purged()) {//判断是否准备删除(will_be_purged)
                    if(prev == NULL) {
                        table[i] = next;
                    } else {
                        prev->set_next(next);
                    ...
                    /* Purge at the next run */   //下一次删除
                    if(head->idle())
                        head->set_to_purge();//标记 will_be_purged = true
