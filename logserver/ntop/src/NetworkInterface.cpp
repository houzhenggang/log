...
NetworkInterface::NetworkInterface(const char *name,
    ...
    if(id >= 0) {
        ...
        running = false,is_idle = false, sprobe_interface = false, inline_interface = false, db = NULL;

        if((!isViewInterface) && (ntop->getPrefs()->do_dump_flows_on_mysql())) {//...mysql
#ifdef NTOPNG_PRO
...
void NetworkInterface::init() {
    ifname = remoteIfname = remoteIfIPaddr = remoteProbeIPaddr = NULL,
            ...
            next_idle_flow_purge = next_idle_host_purge = 0,
            running = false,is_idle = false, numSubInterfaces = 0,
...
//            if(val == 0) is_idle = true;//val=0,s_idle = true时 该接口未使用(闲置状态)
//        }
//    }
...
//        host->serialize2redis();/* possibly dumps counters and data to redis */
...
/* **************************************************** */
/*获取flow返回（从flow_hash查找，未找到new flow）*/
Flow* NetworkInterface::getFlow(u_int8_t *src_eth, u_int8_t *dst_eth,
    ...
    if(ret == NULL) {
        *new_flow = true;
        ...
        if(flows_hash->add(ret)) {//add to flow_hash
...
void NetworkInterface::processFlow(ZMQ_Flow *zflow) {
    ...
    if(zflow->wechat_num )  flow->setWECHATnum(zflow->wechat_num);//byzpl
    if(zflow->qq_num )  flow->setQQnum(zflow->qq_num);//byzpl
    if(zflow->weibo_num )  flow->setWEIBOnum(zflow->weibo_num);//byzpl
...
decode_ip:
    ...
    /*解析传输层是TCP或是UDP或是GRE或是TCP和UDP都不是*/
    ...
    if(!flow->isDetectionCompleted()){
        if(isSampledTraffic())
            ...
            if(!is_fragment) {
                ...
                if(flow->get_packets() >=NDPI_MIN_NUM_PACKETS)//test
                    ...
                    flow->setDetectedProtocol(ndpi_detection_process_packet(ndpi_struct, ndpi_flow,
                                                                            ip, ipsize, (u_int32_t)time,cli, srv), false);
                }
            ...
            if(payload_len > 240) {
                for(int i = 240; i<payload_len; ) {
                    ...
                    if(id == 12 /* Host Name */) {
                        ...
                        // XsDBOp_DhcpCache_InsertMysql(client_mac,name);
    ...
    */
        ...
        case virtual_wangwang:
            flow->dissectWANGWANG();
            break;
        default :
            break;
        }
        //byzpl insert mysql
        //        MyDBOp_InsertDB(flow);//test
    ...
    */
    ...
    //微信视频URL处理
    //    if(flow->get_ndpi_flow()&&flow->get_ndpi_flow()->virtualnum.vedioUrl[0]!='\0'){
    //        flow->virtualnum.id=flow->get_ndpi_flow()->virtualnum.id;
    //        memcpy(flow->virtualnum.vedioUrl, flow->get_ndpi_flow()->virtualnum.vedioUrl,strlen( flow->get_ndpi_flow()->virtualnum.vedioUrl));
    //    }
    //    if(flow->virtualnum.vedioUrl[0] !='\0'){
    //        if(unlikely(0 != Xs_List_mp_PushList(&ntop->listUrl, flow)))
    //            WriteLog("PushList",flow->virtualnum.vedioUrl);
    //    }


    incStats(when->tv_sec, iph ? ETHERTYPE_IP : ETHERTYPE_IPV6,
             flow->get_detected_protocol().app_protocol,
             rawsize, 1, 24 /* 8 Preamb le + 4 CRC + 12 IFG */);
...
void NetworkInterface::purgeIdle(time_t when){
    if(purge_idle_flows_hosts){
        ...
        if((n = purgeIdleFlows()) > 0)//处理flow(是否删除)
            ntop->getTrace()->traceEvent(TRACE_INFO, "Purged %u/%u idle flows on %s",
                                         n, getNumFlows(), ifname);

        if((m = purgeIdleHostsMacsASesVlans()) > 0)//处理 host/mac/autonomous/vlan (是否删除)
...
void NetworkInterface::periodicStatsUpdate() {
    ...
    flows_hash->walk(flow_update_hosts_stats, (void*)&tv);//flow_update_hosts_stats   ntoplog insert mysql start
...
typedef struct _hostfiltered{
    u_int32_t ip_addr;
}HOSTFILTER,*PHOSTFILTER;
...
/* ********************byzpl******************************** */
//search part flow
static bool flow_search_appoint_condition(GenericHashEntry *h, void *user_data){
    ...
    if(retriever->hostFilter.ip_addr!=f->get_cli_ipv4()&&retriever->hostFilter.ip_addr!=f->get_srv_ipv4())
        return(false); /* false = keep on walking */

    retriever->elems[retriever->actNumEntries].flow = f;

    if(1/*f->match(retriever->allowed_hosts)*/) {
        ...
        case column_info:
            ...
            else if(f->getWECHATnum())   retriever->elems[retriever->actNumEntries++].stringValue = f->getWECHATnum();//byzpl
            else if(f->getQQnum())             retriever->elems[retriever->actNumEntries++].stringValue = f->getQQnum();//byzpl
            else if(f->getWEIBOnum())    retriever->elems[retriever->actNumEntries++].stringValue = f->getWEIBOnum();//byzpl
...
//search all flow
static bool flow_search_condition(GenericHashEntry *h, void *user_data) {
    ...
    //        /* Unicast: at least one between client and server is unicast address *///客户机和服务器之间至少有一个是单播地址。
    //        if(retriever->pag
...
//search all mac
static bool mac_search_condition(GenericHashEntry *he, void *user_data) {
...
//search part host
static bool host_search_appoint_condition(GenericHashEntry *he, void *user_data){
    char buf[64];
    struct flowHostRetriever *r = (struct flowHostRetriever*)user_data;
    Host *h = (Host*)he;
    if(r->actNumEntries >= r->maxNumEntries)
    ...
    //        return(false);
    if( /*(r->vlan_id       && (r->vlan_id         != h->get_vlan_id()))                       ||
            ...
            (r->hostFilter.ip_addr!=h->get_ip()->get_ipv4()))
        return(false); /* false = keep on walking */
...
//search all host
static bool host_search_condition(GenericHashEntry *he, void *user_data) {
...
//                               Paginator *p) {
//    struct flowHostRetriever retriever;
//    int (*sorter)(const void *_a, const void *_b);//分类方法
//    char sortColumn[32];
...
//    else if(!strcmp(sortColumn, "column_duration")) retriever.sorter = column_duration, sorter = numericSorter;
//    else if(!strcmp(sortColumn, "column_thpt")) retriever.sorter = column_thpt, sorter = numericSorter;
//    else if((!strcmp(sortColumn, "column_bytes")) || (!strcmp(sortColumn, "column_") /* default */))
...
//    qsort(retriever.elems, retriever.actNumEntries, sizeof(struct flowHostRetrieveList), sorter);//排序
...
//    if(p->a2zSortOrder()) {
//        for(int i=p->toSkip(), num=0; i<(int)retriever.actNumEntries; i++) {//toSkip()???????
//            lua_newtable(vm);
...
//            if(++num >= (int)p->maxHits()) break;//界面一页展示的数据条目数(10/20/50...)
//        }
//    }
...
int NetworkInterface::sortHosts(struct flowHostRetriever *retriever,
    ...
    /* criteria 标准(datatype sortField in ntop_typedefs.h / see also host_search_walker:NetworkInterface.cpp) */
...
/* ***********************byzpl***************************** */
//get part mac
int  NetworkInterface::getPartMacs(PXSJSONMAC json_mac, json_object *pRetRoot){
    struct mac_find_info info;
    int ret;
    memset(&info, 0, sizeof(info));
    //    Utils::parseMac(info.mac, json_mac->mac);
    memcpy(info.mac,json_mac->mac,sizeof(json_mac->mac));
    info.vlan_id = json_mac->vlanID;
    ...
    if(info.m) {
        ...
        info.m->Ret_getJSONObject_Mac(pArray,1);//1 test
        ret = 1;
    } else
        ret = 0;
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
//get mac Info
int NetworkInterface::getAllMacs(PXSJSONMAC json_mac,json_object *pRetRoot){
    ...
    if((json_mac->max_hit > CONST_MAX_NUM_HITS) || (json_mac->max_hit == 0))
        json_mac->max_hit = CONST_MAX_NUM_HITS;
    ...
    // make sure the caller has disabled the purge!!
    walker(walker_macs, mac_search_condition, (void*)&retriever);
    qsort(retriever.elems, retriever.actNumEntries, sizeof(struct flowHostRetrieveList), sorter);

    u_int64_t numMacs=retriever.actNumEntries;//,,,,,,,,,,,,
    u_int32_t i,num;

    for(i=json_mac->to_skip,num=0;i<retriever.actNumEntries;++i){
        Mac *mac=retriever.elems[i].macValue;
        mac->Ret_getJSONObject_Mac(pArray,numMacs);
        if(++num>=json_mac->max_hit) break;
    }
...
/* **************************************************** */
//get part flow info
int NetworkInterface:: getPartFlows(json_object *pRetRoot, PXSJSONFLOW json_flow){
    /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
    //    Host *host = NULL;
    Paginator *p = NULL;
    ...
    //        p->readOptions(vm, 2);
    //        p->readWebOptions(json_flow);

    //    NetworkInterface *iface;
    //    iface->getFlows(vm, get_allowed_nets(vm), host, p);
    ...
    retriever.hostFilter.ip_addr=json_flow->ip_addr;
    ...
    //    snprintf(sortColumn, sizeof(sortColumn), "%s", p->sortColumn());
    retriever.sorter = column_client;//test

    u_int32_t i;
    json_object *pArray=json_object_new_array();

    disablePurge(true);
    walker(walker_flows, flow_search_appoint_condition, (void*)&retriever);
    for(i=0; i<retriever.actNumEntries; ++i){
        Flow *flow=retriever.elems[i].flow;
        flow->Ret_getJSONObject_Flow(pArray,1);//1 test
    }

    json_object_object_add(pRetRoot,"Flow-Info",pArray);
...
//get all flow info
int NetworkInterface::getAllFlows(PXSJSONFLOW json_flow,json_object *pRetRoot){
    Paginator *p = NULL;
    if((p = new(std::nothrow) Paginator()) == NULL)
        return -1;
    ...
    retriever.ndpi_proto = -1;
    retriever.actNumEntries = 0;
    retriever.maxNumEntries =/*10*/ getFlowsHashSize();//test
    /* retriever.allowed_hosts = allowed_hosts*/
    retriever.elems = (struct flowHostRetrieveList*)calloc(sizeof(struct flowHostRetrieveList), retriever.maxNumEntries);
    ...
    //    snprintf(sortColumn, sizeof(sortColumn), "%s", p->sortColumn());
    retriever.sorter = column_bytes,sorter = numericSorter;//test
    ...
    json_object *pArray=json_object_new_array();
    disablePurge(true);
    walker(walker_flows, flow_search_condition, (void*)&retriever);
    qsort(retriever.elems, retriever.actNumEntries, sizeof(struct flowHostRetrieveList), sorter);//排序

    u_int64_t numFlows=retriever.actNumEntries;//,,,,,,,,,,,,

    u_int32_t i,num;
    for(i=json_flow->to_skip,num=0; i<retriever.actNumEntries; ++i) {
        Flow *flow=retriever.elems[i].flow;
        flow->Ret_getJSONObject_Flow(pArray,numFlows);
        if(++num >=json_flow->max_hit) break;//.............................
    }
    json_object_object_add(pRetRoot,"Flow-Info",pArray);
...
//get part host info
int NetworkInterface::getPartHosts( json_object *pRetRoot,PXSJSONHOST json_host){
    struct flowHostRetriever retriever;

    u_int32_t maxHits = getHostsHashSize();
    if((maxHits > CONST_MAX_NUM_HITS) || (maxHits == 0))
        maxHits = CONST_MAX_NUM_HITS;

    retriever.actNumEntries = 0;
    retriever.maxNumEntries = maxHits;
    retriever.hostFilter.ip_addr=json_host->ip_addr.s_addr;//test
    ...
    retriever.sorter = column_ip;//test
    ...
    // make sure the caller has disabled the purge!!
    walker(walker_hosts, host_search_appoint_condition, (void*)&retriever);
    u_int32_t i;
    for(i=0;i<retriever.actNumEntries;++i){//2147554476(128)  1920103026(114)
        Host *host=retriever.elems[i].hostValue;
        host->Ret_getJSONObject_Host(pArray,1);//1test
    }

    json_object_object_add(pRetRoot,"Host-Info",pArray);
...
//get all  host info
int NetworkInterface::getAllHosts( PXSJSONHOST json_host,json_object *pRetRoot){
    /*init*/
    ...
    LocationPolicy location =location_all;//test
    bool hostMacsOnly = false;//all macs
    ...
    retriever.vlan_id = vlan_filter;
    retriever.osFilter = os_filter;
    retriever.asnFilter = asn_filter;
    retriever.networkFilter = network_filter;
    retriever.actNumEntries = 0;
    retriever.poolFilter = pool_filter;
    retriever.bridge_iface_idx = 0;
    retriever.ipVersionFilter = ipver_filter;
    retriever.filteredHosts = filtered_hosts;
    retriever.ndpi_proto = proto_filter;
    retriever.maxNumEntries = maxHits;//10
    ...
    // make sure the caller has disabled the purge!!
    walker(walker_hosts, host_search_condition, (void*)&retriever);

    qsort(retriever.elems, retriever.actNumEntries, sizeof(struct flowHostRetrieveList), sorter);

    u_int64_t numhosts=retriever.actNumEntries;
    u_int32_t i,num;
    for(i=json_host->to_skip,num=0;i<retriever.actNumEntries;++i){
        Host *host=retriever.elems[i].hostValue;
        host->Ret_getJSONObject_Host(pArray,numhosts);
        if(++num>=json_host->max_hit) break;
    }
    json_object_object_add(pRetRoot,"Host-Info",pArray);
    enablePurge(false);


    // it's up to us to clean sorted data清理分类数据
    // make sure first to free elements in case a string sorter has been used确保使用字符串分类器时释放元素
...
int NetworkInterface::getAllInterfaceStats( json_object *pRetRoot){
    json_object *pArray=json_object_new_array();
    Ret_getJSONObject_Interface(pArray);
    json_object_object_add(pRetRoot,"Interface-Info",pArray);
}
/* **************************************************** */
...
/* **************************************************** */
/*获取各子网的主机的活动情况*/ //对应后台host/networks
...
//    // sort hosts according to the grouping criterion 按分组标准排序主机
...
//    // build a new grouper that will help in aggregating stats建立一个新的组，有助于统计数据
...
////    Lua::luaRegisterInternalRegs(L);//注册Lua相关函数
