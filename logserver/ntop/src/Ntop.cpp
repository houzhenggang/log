...
Ntop::Ntop(char *appName) {
    ...
    g_xsDebug=0;
    //牵引模式
    rb_root_init(&mapXsWork);
    //镜像模式
    Xs_List_Init(&ntop->listUrl);
    rb_root_init(&ntop->mapPartition);
    rb_root_init(&ntop->mapPartitionUrl);
    rb_root_init(&ntop->mapPartitionPicFile);
    rb_root_init(&ntop->mapPartitionFileDw);
    rb_root_init(&ntop->mapPartitionDhcp);
    rb_root_init(&ntop->mapInterfaceName);
    rb_root_init(&ntop->mapInterfaceID);
    //    rb_root_init(&ntop->mapPartitionDns);
    //    rb_root_init(&ntop->mapPartitionID);
    //    Xs_List_Init(&ntop->list);
    //    int res=sem_init(&ntop->bin_sem,0,0);
    ...
    memcpy(g_dbip,"127.0.0.1",strlen("127.0.0.1"));
    memcpy(g_sip,"10.127.122.63",sizeof("10.127.122.63"));

    globals = new NtopGlobals();//new 全局信息init
    pa = new PeriodicActivities();//线程活动init
    address = new AddressResolution();//
    httpbl = NULL, flashstart = NULL;//CML -k
    custom_ndpi_protos = NULL;//CML -p
    prefs = NULL/*, redis = NULL*/;
    elastic_search = NULL;
    logstash = NULL;
    num_cpus = -1;
    num_defined_interfaces = 0;//注册可用接口num
    ...
    //printf("--> %s [%s]\n", startup_dir, appName); //working directory
...
Ntop::~Ntop() {
    ...
    if(httpCMulti) delete httpCMulti;

    if(elasticsearch) delete (elasticsearch);
    if(pIfMsg) {
        free(pIfMsg);
        pIfMsg = NULL;
    }

    int_void_rbdestroy(&mapXsWork);

    int_void_rbdestroy(&ntop->mapPartition);
    int_void_rbdestroy(&ntop->mapPartitionUrl);
    int_void_rbdestroy(&ntop->mapPartitionPicFile);
    int_void_rbdestroy(&ntop->mapPartitionFileDw);
    int_void_rbdestroy(&ntop->mapPartitionDhcp);
    Xs_List_Destroy(&ntop->listUrl,free);
    int_void_rbdestroy(&ntop->mapInterfaceName);
    int_void_rbdestroy(&ntop->mapInterfaceID);
    //    sem_destroy(&ntop->bin_sem);
    //    Xs_List_Destroy(&ntop->list,free);
    //    int_void_rbdestroy(&ntop->mapPartitionDns);
    //    int_void_rbdestroy(&ntop->mapPartitionID);

    Xs_Idc_Destroy(&g_xsIdc);
...
void Ntop::initElasticSearch() {
    ...
    elastic_search = new ElasticSearch();//init
}
...
void Ntop::MyDBOp_Insert_Interface_Info(){
    //1.mysql
    //    char sql[128];
    //    for(int i=0; i<num_defined_interfaces; i++){
    //        memset(sql,0,sizeof(sql));
    //        sprintf(sql,"insert into tbntopinterface(ifName,ifID) values ('%s',%u)",iface[i]->get_name(),iface[i]->get_id());
    ...
    for(int i=0; i<num_defined_interfaces; i++){
        if(node) continue;
        pIf_Msg ifmsg =(pIf_Msg)malloc(IF_MSG_SIZE);//notice free
        memcpy(ifmsg->ifname,iface[i]->get_name(),sizeof(iface[i]->get_name()));
        ifmsg->ifID = iface[i]->get_id();
        ifmsg->hashkey = CRC64((u_char *)ifmsg->ifname,strlen(ifmsg->ifname));
        node = int_void_rbinsert(&mapInterfaceName,ifmsg->hashkey,ifmsg);
        node = int_void_rbinsert(&mapInterfaceID,ifmsg->ifID,ifmsg);
    }

    //3.Es
    //    for(int i=0; i<num_defined_interfaces; i++){
    //        XsPostInterfaceToES(iface[i]->get_name(),iface[i]->get_id());
    //    }

}

void MyDBOp_Deal_IfaceInfo(){
    /*~~清空表数据（tbntopinterface）~~*/
    //MyDBOp_Clear_Interface_Info(/*&ntop->g_xsMySql*/);
    /*~~将接口插入数据库（tbntopinterface）~~*/
    ntop->MyDBOp_Insert_Interface_Info();
}
/* ******************************************* */
...
void Ntop::start() {
    ...
    //MyDBOp_Deal_IfaceInfo();

    Xs_Idc_Start();//拉取协议

    XsCreate_RcvThread_WithPort();//牵引模式，根据端口创建多线程收包 // GetLogMsg_InsertMysql_FromXsdpi();

    //    ntop->getES()->XsPostEsData();//test
    ntop->getES()->startEsDump();//strat elasticsearch

    //    Xsweb_Work(); //Web接口线程(将flow信息以json返回)

    pthread_t downHttp;
    err=pthread_create(&downHttp,0,MainTain_Thread,NULL);//MainTain_Thread
    if(err!=0) printf("Create MainTain_Thread Failed. %s\n", strerror(err));
    close(downHttp);

    //    if(httpCMulti)//暂时不启用该下载
    //        httpCMulti->Run();
    ...
    //   pa->startPeriodicActivitiesLoop();//脚本运行等前期配置
    // if(httpbl) httpbl->startLoop();
    /*  else*/ if(flashstart) flashstart->startLoop();

    //runtimeprefs = new RuntimePrefs();//运行配置 （大多与ntop 界面参数配置相关）
    ...
    for(int i=0; i<num_defined_interfaces; i++) {
        ...
        iface[i]->startPacketPolling();//抓包分析
    }
    ...
    //    pthread_t t;
    //    err= pthread_create(&t,NULL,ThreadInsertVedioUrlToMysql,NULL);
    //    if(err!=0) printf("Create ThreadInsertVedioUrlToMysql Failed. %s\n", strerror(err));
    //    close(t);
    ...
    while(!globals->isShutdown()) {
        ...
        runHousekeepingTasks();//..........................write mysql(tbntoplog)
...
//        return true; /* always return true when no lua state is passed *///总是返回true时没有Lua状态
//    }
...
void Ntop::setLocalNetworks(char *_nets) {//_nets:127.0.0.0/8
    char *nets;
    u_int len;
...
void Ntop::shutdown() {
    for(int i=0; i<num_defined_interfaces; i++) {
        ...
        stats->print();
        fprintf(stdout,"before shutdown \n");
        iface[i]->shutdown();
        fprintf(stdout,"after shutdown \n");
