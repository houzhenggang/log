...
/* **************************************************** */
#define  Interface_Index_Head  "{\"index\": {\"_type\": \"ifmsg\", \"_index\": \"interface\",\"_search\":{\"ifName\":\"%s\"}}}"
#define Interface_Index_Url "http://localhost:9200/interface/ifmsg"
int Utils::ifname2id(const char *name){
    int i=0;
    //    u_int16_t ifaceID;
    //    PMYDBOP pDB=&ntop->g_xsMySql;
    ...
    struct rb_node *node;
    u_int64_t hashkey = 0;
    hashkey = CRC64((u_char *)name,strlen(name));

    node = int_void_rbsearch(&ntop->mapInterfaceName,hashkey);
    if(node){
        ntop->pIfMsg =(pIf_Msg) node->data;
        return ntop->pIfMsg->ifID;
    }else{
        for(int idx=0; idx<255; idx++){
            node = int_void_rbsearch(&ntop->mapInterfaceID,idx);
            if(NULL == node){
                ntop->pIfMsg=(pIf_Msg)malloc(IF_MSG_SIZE);//notice free

                memcpy(ntop->pIfMsg->ifname,name,sizeof(name));
                ntop->pIfMsg->ifID = idx;
                ntop->pIfMsg->hashkey = hashkey;

                int_void_rbinsert(&ntop->mapInterfaceName,hashkey,ntop->pIfMsg);
                int_void_rbinsert(&ntop->mapInterfaceID,idx,ntop->pIfMsg);
    ...
    //~~~~~~~~~~~~~~~~~
    //    char sql[128]={0};
    //    sprintf(sql,"select ifID from tbntopinterface where ifName='%s';",name);

    //    MYSQLRECORDSET GetQueryID={0};
    //    CSqlRecorDset_Init(&GetQueryID);
    //    CSqlRecorDset_SetConn(&GetQueryID,pDB->m_pDB);
    //    CSqlRecorDset_CloseRec(&GetQueryID);
    //    CSqlRecorDset_ExecSQL(&GetQueryID, sql);

    //    if(CSqlRecorDset_GetRecordCount(&GetQueryID)==0) {
    //        //如果数据库没有找到将其插入数据库，并返回该id
    //        for(u_int idx=0; idx<255; idx++){
    //            sprintf(sql,"select ifID from tbntopinterface where ifID=%d;",idx);
    //            CSqlRecorDset_ExecSQL(&GetQueryID, sql);
    //            if(CSqlRecorDset_GetRecordCount(&GetQueryID)==0){
    //                sprintf(sql,"insert into tbntopinterface(ifName,ifID) values ('%s',%u)",name,idx);
    //                if(!MyDBOp_ExecSQL_1(&ntop->g_xsMySql,sql))
    //                    WriteLog("writemysql",sql);
    //                CSqlRecorDset_CloseRec(&GetQueryID);
    //                CSqlRecorDset_Destroy(&GetQueryID);
    //                return idx;
    //            }else
    //                continue;
    //        }
    //    }else {
    //        for(i=0; i< CSqlRecorDset_GetRecordCount(&GetQueryID);i++)
    //        {
    //            CSqlRecorDset_GetFieldValue_U16(&GetQueryID,"ifID",&ifaceID);
    //            CSqlRecorDset_CloseRec(&GetQueryID);
    //            CSqlRecorDset_Destroy(&GetQueryID);
    //            return ifaceID;
    //        }
...
//                                         url, curl_easy_strerror(res));
        ...
        curl_easy_cleanup(curl);//清除easy curl对象
    }
...
//                 printf("RESPONSE_CODE:%s",content_type);
...
//                 printf("RESPONSE_CODE:%s",redirection);
