...
void Host::set_host_label(char *label_name, bool ignoreIfPresent) {
    ...
    if(label_name){
        ...
        // res=XsDBOp_Query_Host_Label(host,host_label);
        //        if(ignoreIfPresent&&res)
        //            return;
        //        else
        //            XsDBOp_HostLabel_InsertMysql(host,label_name);
    }
    /* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
    //    if(label_name) {
...
void Host::initialize(u_int8_t _mac[6], u_int16_t _vlanId, bool init_all) {
    ...
    dns = NULL, http = NULL, /*wechat=NULL,byzpl*/categoryStats = NULL, top_sites = NULL, old_sites = NULL,
            icmp = NULL;
    ...
    if(init_all) {
        ...
        //            else if(!ntop->getRedis()->get(redis_key, json, HOST_MAX_SERIALIZED_LEN)){//... 获取json内容
...
bool Host::readDHCPCache() {
    if(localHost && mac && (!dhcpUpdated)) {
        ...
        if(!mac->isNull()) {
           ...
           // XsDBOp_Query_Dhcp_Cache(client_mac,buf);
            if(buf[0]!='\0'){
                setName(buf);
                return true;
            }
            ...
            //            if(ntop->getRedis()->hashGet(key, client_mac, buf, sizeof(buf)) == 0){// key :tablename
            //                setName(buf);
            //                return true;
            //            }
        }
...
//    lua_newtable(vm);//新建并压入一个表
//    /*~...一些（int，string...）入栈（lua_settable（）弹出）操作~*/
...
//    lua_push_str_table_entry(vm, "name", get_visual_name(buf, sizeof(buf)));//终端名称
...
//    lua_push_str_table_entry(vm, "os", os);//操作系统
...
/* *************************************** */
/*~...将数据转存到redis(将json数据内容写进对应数据库表中)~*/
...
json_object* Host::getJSONObject() {
    ...
    /*~~~~~~~~~~byzpl~~~~~~~~~~~~~~*/
    //  if(wechat) json_object_object_add(my_object,"wechat",wechat->getJSONObject());
    /*~~~~~~~~~~~~~~~~~~~~~~*/
    return(my_object);
}
...
bool Host::deserialize(char *json_str, char *key) {
    ...
    /*~...json_object_object_get_ex()获取与给定对象字段关联的json_object;json_object_get_int64()如果传递的对象不是int64，则该类型被强制转换为int64~*/
    ...
    /*~byzpl~*/
    //    if(json_object_object_get_ex(o,"wechat",&obj)){
    //        if(wechat) wechat->deserialize(obj);
    //    }
