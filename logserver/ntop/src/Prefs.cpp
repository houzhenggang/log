...
Prefs::Prefs(Ntop *_ntop) {
    ...
    num_interfaces = 0, enable_auto_logout = true/*logout detele*/;
    ...
    es_type = strdup((char*)"flows"), es_index = strdup((char*)"sxzlog-%Y.%m.%d"),
            es_url = strdup((char*)"http://localhost:9200/_bulk"),//localhost
...
void Prefs::reloadPrefsFromRedis(){
    ...
    //尝试加载从Web UI设置的首选项，并在未找到的情况下应用默认值
    ...
    getDefaultPrefsValue(CONST_RUNTIME_IS_AUTOLOGOUT_ENABLED,/*logout detele*/
...
/* ******************************************************** */
   // setTraceLevelFromRedis();//default    replace  set_logging_level
...
{"recv-xsdpi-log-port",                           required_argument,      NULL,    'P'},//byzpl
{"printf-all-debug-msg",                          required_argument,     NULL,   'c'   },
{"set-log-level",                                     required_argument,     NULL,    'f'},
{  "daemon_mode_running",                   required_argument,     NULL,       'a'   },
...
int Prefs::setOption(int optkey, char *optarg) {
    ...
    case 'q':
        enable_auto_logout = false;/*logout detele*/
        break;
    ...
    case 'F':
        if((strncmp(optarg, "es", 2) == 0) && (strlen(optarg) > 3)) {
        ...
        } else if(!strncmp(optarg, "mysql", 5)) {//notice mysql
            ...
            optarg = Utils::tokenizer(optarg, ';', &mysql_tablename);  // default exist
...
#endif
        /* ~~~~~~~~~~~~~~~~~byzpl~~~~~~~~~~~~~~~~~~~~~~~ */
        /* **c:打印debug信息, P:指定日志接收端口号(xsdpi)   f:设置是否打印ntop日志（level）** */
//    case 'P':
//        ntop->port=atoi(optarg);
//        printf("port %d\n",ntop->port);
//        break;
    case 'c':
        ntop->g_xsDebug=atoi(optarg);
        break;
    case 'f':
        set_logging_level((u_int8_t)atoi(optarg));
        break;
    case 'a':
       daemon_mode=1;
        break;
...
int Prefs::loadFromCLI(int argc, char *argv[]) {
    ...
    while((c = getopt_long(argc, argv,
                           "k:eg:hi:w:r:sg:m:n:p:qd:t:x:1:2:3:l:uv:A:B:CD:E:F:N:G:HI:O:Q:S:TU:X:W:VZ:P:c:f:a:",
                           long_options, NULL)) != '?') {
        if(c == 255) break;
        setOption(c, optarg);
    }
...
//    lua_push_bool_table_entry(vm, "is_autologout_enabled", enable_auto_logout);/*logout detele*/
