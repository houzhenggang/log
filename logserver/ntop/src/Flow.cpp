...
time_t _first_seen, time_t _last_seen) : GenericHashEntry(_iface) {
    ...
    flag=0;//byzpl
    virtualnum.hashkey=0;
    virtualnum.id=0;
    memset(virtualnum.szNum,0,sizeof(virtualnum.szNum));
    memset(virtualnum.szUrl,0,sizeof(virtualnum.szUrl));
    memset(virtualnum.vedioUrl,0,sizeof(virtualnum.vedioUrl));
    memset(virtualnum.mail_rcv,0,sizeof(virtualnum.mail_rcv));
    ...
    wechat.id=NULL; qq.id=NULL,weibo.id=NULL,e_mail.mailId=NULL,wangwang.id=NULL;//byzpl
...
Flow::~Flow() {
    ...
    if(isHTTP()) {
        ...
        if(protos.http.last_host) {
            free(protos.http.last_host);
            protos.http.last_host=NULL;
        }//byzpl...
    } else if(isWECHAT()){//byzpl
        if(wechat.id)    free(wechat.id);
    }else if(isQQ()){//byzpl
        if(qq.id)    free(qq.id);
    }else if(isWEIBO()){//byzpl
        if(weibo.id)    free(weibo.id);
    }else if(isDNS()) {
    ...
    case NDPI_PROTOCOL_HTTP_PROXY:
        if(ndpiFlow->host_server_name[0] != '\0') {
            ...
            if(srv_host && (ndpiFlow->detected_os[0] != '\0') && cli_host) {
                ...
                //                ++count_os;
                //                fprintf(stdout,"OS:[%s]  Count[%u]\n",ndpiFlow->detected_os,count_os);
...
#endif
    /*~~~~~~~~~~~~~~~~~~byzpl(将ndpi_flow_struct(private)赋值至Flow  class  VirtualNum  (public))~~~~~~~~~~~~~~~~*/
    if(ndpiFlow){
        /* **(qqnum/wechatnum/weibonum/email)** */
        virtualnum.id=ndpiFlow->virtualnum.id;
        virtualnum.type=ndpiFlow->virtualnum.type;
        virtualnum.status=ndpiFlow->virtualnum.status;
        virtualnum.wangVer=ndpiFlow->virtualnum.wangVer;
        //mail/wangwang
        virtualnum.proType=ndpiFlow->virtualnum.proType;
        if(strlen(ndpiFlow->virtualnum.szNum))
            memcpy(virtualnum.szNum,ndpiFlow->virtualnum.szNum,strlen(ndpiFlow->virtualnum.szNum));//sender
        if(strlen(ndpiFlow->virtualnum.mail_rcv))
            memcpy(virtualnum.mail_rcv,ndpiFlow->virtualnum.mail_rcv,strlen(ndpiFlow->virtualnum.mail_rcv));//receiver
        //qq_msg  file download....
        virtualnum.TypeMsg=ndpiFlow->virtualnum.TypeMsg;//
        virtualnum.hashkey=ndpiFlow->virtualnum.hashkey;
        if(strlen(ndpiFlow->virtualnum.szUrl))
            memcpy(virtualnum.szUrl,ndpiFlow->virtualnum.szUrl,strlen(ndpiFlow->virtualnum.szUrl));
        /* **(http_host)** */
        if(isHTTP()&&ndpiFlow->packet.host_line.len>0){
            if(protos.http.last_host){
                free(protos.http.last_host);
                protos.http.last_host=NULL;
            }
            u_int16_t len =ndpiFlow->packet.host_line.len;
            //            protos.http.last_host=NULL;
            protos.http.last_host=(char *)malloc(len+1);//notice  free
            memset(protos.http.last_host,0,len+1);
            if(len)
                memcpy(protos.http.last_host,ndpiFlow->packet.host_line.ptr,len);
        }/*else{
            protos.http.last_host=NULL;
        }*/
        flag=1/*ndpiFlow->flag*/;

        /*~~~~~~~~~~~test~~~~~~~~~~~~~~~~~~~~*/
        switch(virtualnum.TypeMsg){
        case msg_qq_picture:
        case msg_qq_video:
        case msg_qq_file:
        case msg_wechat_picture:
        case msg_wechat_video:
            XsHttp_Add_Down_URL(this);
            break;
        default:
            break;
        }
...
void Flow::setDetectedProtocol(ndpi_protocol proto_id, bool forceDetection) {
    if(proto_id.app_protocol != NDPI_PROTOCOL_UNKNOWN) {
        struct ndpi_flow_struct *ndpi_flow = this->get_ndpi_flow();
...
bool Flow::dumpFlow(bool idle_flow) {
    ...
    /*~~~~~~byzpl(record ntop logmsg to  mysql)~~~~*/
//    if(flag){
//        dumpMysqlFlow(this);
//        flag=0;
//    }
    ...
    if(ntop->getPrefs()->do_dump_flows_on_mysql()
        ...
        if(cli_host) {
            if(ntop->getPrefs()->do_dump_flows_on_mysql())
                cli_host->getInterface()->dumpDBFlow(last_seen, this);//(...dump flow to mysql)
...
//        {
//            lua_push_str_table_entry(vm, "proto.ndpi","AliWangWang");//...
//        }
//            break;
//        default:
//        {
...
//                lua_push_str_table_entry(vm, "proto.ndpi",get_detected_protocol_name(buf, sizeof(buf)));//.........application
//            } else
//                lua_push_str_table_entry(vm, "proto.ndpi", (char*)CONST_TOO_EARLY);}
//            break;
//        }

//        lua_push_int_table_entry(vm, "proto.ndpi_id", ndpiDetectedProtocol.app_protocol);//...搜索栏application
...
//        //            lua_push_str_table_entry(vm, "info",buffer);
//        //            //            lua_push_str_table_entry(vm, "info", getFlowInfo() ? getFlowInfo() : (char*)"");//default...
//        //        }
...
//                lua_push_str_table_entry(vm, "protos.wechat.id", wechat.id);
//            if(isQQ()&&qq.id)
//                lua_push_str_table_entry(vm, "protos.qq.id", qq.id);
//            if(isWEIBO()&&weibo.id)
//                lua_push_str_table_entry(vm, "protos.weibo.id", weibo.id);
//            if(e_mail.mailId)
//                lua_push_str_table_entry(vm, "protos.email.id",e_mail.mailId);
//            if(wangwang.id)
//                lua_push_str_table_entry(vm, "protos.email.id",wangwang.id);
//            /*~~~~~~~~~~~~~~~~~~~~~~~~~~*/
...
json_object* Flow::flow2json() {
    ...
    */
    ...
    /*~~~~~~~byzpl~~~~~~~*/
    if(isWECHAT()&&wechat.id){
        json_object_object_add(my_object, "WECHAT_NUM", json_object_new_string(wechat.id));
    }
    if(isQQ()&&qq.id){
        json_object_object_add(my_object, "QQ_NUM", json_object_new_string(qq.id));
    }
    if(isWEIBO()&&weibo.id){
        json_object_object_add(my_object, "WEIBO_NUM", json_object_new_string(weibo.id));
    }
    if(e_mail.mailId){
        json_object_object_add(my_object, "MAIL_NUM", json_object_new_string(e_mail.mailId));
    }
    if(wangwang.id){
        json_object_object_add(my_object, "WANGWANG_NUM", json_object_new_string(wangwang.id));
    }
    /*~~~~~~~~~~~~~~~~~~~~*/
    if(bt_hash)
...
/* *************************************** */
/* *~~~~~~byzpl(only recode  url/qq/wechat/weibo/email  flow )~~~~~~~* */
void get_VirtualnumStatusString(char * act,int status){
    switch(status){
    case vrt_method_offline :
        memcpy(act,"OFFLINE",7);
        break;
    case  vrt_method_online :
        memcpy(act,"ONLINE",6);
        break;
    default:
        break;
...
int Flow::getFlowInfoToMysql(char *buffer,int size,char *type,char *act,char * mac,u_int date)
{
    if(virtualnum.hashkey){
        switch(virtualnum.TypeMsg) {
        case msg_wechat_video:
        {
            if(wechat.id)
                memcpy(buffer,wechat.id,MIN(strlen(wechat.id),size));
            else
                memcpy(buffer," ",size);
            get_VirtualnumStatusString(act,virtualnum.status);
            memcpy(type,"wechat_video",12);
            return 2;//break;
        }
        case msg_wechat_picture:{
            if(wechat.id)
                ...
                memcpy(buffer," ",size);
            get_VirtualnumStatusString(act,virtualnum.status);
            memcpy(type,"wechat_picture",14);
            return 2;//break;
        }
        case msg_qq_video:{
            if(qq.id)
                memcpy(buffer,qq.id,MIN(strlen(qq.id),size));
            else
                memcpy(buffer," ",size);
            get_VirtualnumStatusString(act,virtualnum.status);
            memcpy(type,"qq_video",8);
            return 2;//break;
        }
        case msg_qq_file:{
            if(qq.id)
        ...
        case msg_qq_picture:{
            if(qq.id)
                ...
                memcpy(buffer," ",size);
            get_VirtualnumStatusString(act,virtualnum.status);
            memcpy(type,"qq_picture",strlen("qq_picture"));
            return 2;//break;
        }
        default:
            break;
    ...
    if(wechat.id){
        ...
        memcpy(buffer,weibo.id,MIN(strlen(weibo.id),size));
        get_VirtualnumStatusString(act,virtualnum.status);
        memcpy(type,"weibolog",8);
        return 1;
    }
    else if(wangwang.id){
        memcpy(buffer,wangwang.id,MIN(strlen(wangwang.id),size));
        memcpy(type,"wangwanglog",11);
        //        get_VirtualnumStatusString(act,virtualnum.status);
        if(virtualnum.wangVer==buyer_version)
            memcpy(act,"buyer_version",14);//aoao
        else if(virtualnum.wangVer==seller_version)
            memcpy(act,"seller_version",14);
        else
            get_VirtualnumStatusString(act,virtualnum.status);
        return 1;
    }
    else if(e_mail.mailId){
        if(virtualnum.proType==mail_pop){
            memcpy(buffer,e_mail.mailId,MIN(strlen(e_mail.mailId),size));
            memcpy(type,"pop_mail",8);//memcpy(type,"emaillog",9);
            // memcpy(act,"POP",3);// memcpy(act,"POP",3);
            get_VirtualnumStatusString(act,virtualnum.status);
            return 1;
        }
        else if(virtualnum.proType==mail_smtp){
            if(e_mail.mailRcvId){
                char Str[128];
                snprintf(Str,127,"FROM:%s TO:%s",e_mail.mailId,e_mail.mailRcvId);
                memcpy(buffer,Str,MIN(strlen(Str),size));
                memcpy(type,"smtp_mail",9);
                // memcpy(act,"SMTP",4);
                get_VirtualnumStatusString(act,virtualnum.status);
                return 1;
            }else{
        ...
        else if(virtualnum.proType==mail_imap){
            memcpy(buffer,e_mail.mailId,MIN(strlen(e_mail.mailId),size));
            memcpy(type,"imap_mail",9);
            // memcpy(act,"IMAP",4);
            get_VirtualnumStatusString(act,virtualnum.status);
            return 1;
        }
    ...
    if(!isMaskedFlow()) {
        if(isHTTP() && protos.http.last_method && protos.http.last_url&&srv_host){//url
            if(protos.http.last_host!=NULL){
                snprintf(buffer,size,"http://%s%s",protos.http.last_host,protos.http.last_url);//type:host
            }else {
                char ipbuf[32]={0};
                snprintf(buffer,size,"http://%s:%d%s",srv_host->get_ip()->print(ipbuf,sizeof(ipbuf)),ntohs(srv_port),protos.http.last_url);//type:ip
            }
            memcpy(type,"urllog",6);
            memcpy(act,protos.http.last_method,10);
            return 0;
        }
    }
...
char* Flow::getFlowInfo(char *buffer,int size) {
    if(!isMaskedFlow()) {
        if(isDNS() && protos.dns.last_query){
            //            return protos.dns.last_query;
            memcpy(buffer,protos.dns.last_query,MIN(strlen(protos.dns.last_query),size));
            return NULL;
        }
        //        else if(isHTTP() && protos.http.last_method && protos.http.last_url)
        //            return protos.http.last_url;
        else if(isHTTP() && protos.http.last_method && protos.http.last_url&&srv_host){//url
            if(protos.http.last_host!=NULL){
                snprintf(buffer,size,"http://%s%s",protos.http.last_host,protos.http.last_url);//type:host
            }else {
                char ipbuf[32]={0};
            ...
            memcpy(buffer,protos.ssl.certificate,MIN(strlen(protos.ssl.certificate),size));
            // return protos.ssl.certificate;
            return NULL;
        }
        else if(bt_hash){
            memcpy(buffer,bt_hash,MIN(strlen(bt_hash),size));
            //           return bt_hash;
            return NULL;
        }
        else if(host_server_name){
            memcpy(buffer,host_server_name,MIN(strlen(host_server_name),size));
            //  return host_server_name;
            return NULL ;
        }
        else if(isSSH()) {
            if(protos.ssh.server_signature){
                memcpy(buffer,protos.ssh.server_signature,MIN(strlen(protos.ssh.server_signature),size));
                //                return protos.ssh.server_signature;
                return NULL;
            }
            else if(protos.ssh.client_signature){
                memcpy(buffer,protos.ssh.client_signature,MIN(strlen(protos.ssh.client_signature),size));
        ...
        if(payload && ((space = strchr(payload, ' ')) != NULL)) {
            ...
            if((!strncmp(payload, "GET", 3))
                ...
                if((protos.http.last_method = (char*)malloc(l+1)) != NULL) {
                    ...
                    protos.http.last_method[l] = '\0';//...http type(GET,PUT,..)
                }
                ...
                if((space = strchr(payload, ' ')) != NULL) {
                    ...
                    /* Stop at the first non-printable char of the HTTP URL 停止在HTTP URL的第一个不可打印的字符*/
                    for(u_int i=0; i<l; i++) {
        ...
        if(dissect_next_http_packet) {
            ...
            if((space = strchr(payload, ' ')) != NULL) {
                ...
                if((space = strchr(payload, ' ')) != NULL) {
                    ...
                    strncpy(tmp, payload, l);//...tmp->http reply code(200)
                    tmp[l] = 0;
                    protos.http.last_return_code = atoi(tmp);
                }
            }

            // Detect content type in response header检测响应头中的内容类型
...
/* *************************************** */
/*~~~~~~~~~byzpl(qq/wechat/weibo)~~~~~~~~~*/
void Flow::dissectWECHAT(){

    if(wechat.id){
        free(wechat.id);
        wechat.id = NULL;
    }
    wechat.id=(char *)malloc(32);
    memset(wechat.id,0,32);
    snprintf(wechat.id,31,"%lu",virtualnum.id);
}

void Flow::dissectWANGWANG(){

    if(wangwang.id){
        free(wangwang.id);
        wangwang.id = NULL;
    }
    wangwang.id=(char *)malloc(32);
    memset(wangwang.id,0,32);
    snprintf(wangwang.id,31,"%s",virtualnum.szNum);
}


void Flow::dissectWEIBO(){

    if(weibo.id){
        free(weibo.id);
        weibo.id = NULL;
    }
    weibo.id=(char *)malloc(32);
    memset(weibo.id,0,32);
    snprintf(weibo.id,31,"%lu",virtualnum.id);
}
...
void Flow::dissectQQ(){
    ...
    qq.id=(char *)malloc(32);
    memset(qq.id,0,32);
    snprintf(qq.id,31,"%lu",virtualnum.id);
}

void Flow::dissectMAIL(){
    //sender mail
    if(e_mail.mailId){
        free(e_mail.mailId);
        e_mail.mailId= NULL;
    }
    e_mail.mailId=(char *)malloc(32);
    memset(e_mail.mailId,0,32);
    snprintf(e_mail.mailId,31,"%s",virtualnum.szNum);

    //Receiver mail
    if(e_mail.mailRcvId){
        free(e_mail.mailRcvId);
        e_mail.mailRcvId= NULL;
    }
    e_mail.mailRcvId=(char *)malloc(32);
    memset(e_mail.mailRcvId,0,32);
    snprintf(e_mail.mailRcvId,31,"%s",virtualnum.mail_rcv);
}
/* *************************************** */
