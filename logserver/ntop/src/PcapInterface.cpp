...
PcapInterface::PcapInterface(const char *name) : NetworkInterface(name) {
    ...
    //抓包
    if((/*1 ||*/stat(name, &buf) == 0) || (name[0] == '-') || !strncmp(name, "stdin", 5)) {//byzpl...test
   /*
    ...
    */
        ...
        else if((pcap_handle = pcap_open_offline("/root/xslogupdata/Build/Debug/bin/wangwang.pcapng"/*ifname*/, pcap_error_buffer)) == NULL) {
...
static void* packetPollLoop(void* ptr) {
    ...
    prctl(PR_SET_NAME,iface->get_name());
    ...
    do {
        ...
        while((pd != NULL)
            ...
            while(iface->idle()) {
                iface->purgeIdle(time(NULL)); sleep(1);
            }
      ...
      */
                    ...
                    iface->dissectPacket(0, hdr, pkt, &p, &srcHost, &dstHost, &flow);//包处理
#endif
                }
            } else if(rc < 0) {
                ...
                /* No packet received before the timeout */ //在超时之前没有收到数据包。
