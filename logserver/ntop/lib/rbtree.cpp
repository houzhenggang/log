...
void rb_insert_color(struct rb_node *node, struct rb_root *root)
{
    ...
    __sync_fetch_and_add(&root->ncount,1);
}
...
color:
    ...
    __sync_fetch_and_sub(&root->ncount,1);
}
...
struct rb_node *rb_first(struct rb_root *root)
{
    ...
    if (!n)
        return NULL;
    struct rb_node *rb_left=n->rb_left;
    while(rb_left) {
        n = rb_left;
        rb_left = n->rb_left;
    }
...
struct rb_node *rb_last(struct rb_root *root)
{
    ...
    n = root->rb_node;  //empty root
    if (!n)
        return NULL;
     struct rb_node *rb_righ=n->rb_right;
     while(rb_righ) {
         n = rb_righ;
         rb_righ = n->rb_right;
     }
...
struct rb_node *rb_next( struct rb_node *node)
{
    ...
    if(RB_EMPTY_NODE(node))//if (rb_parent(node) == node)
        return NULL;
    ...
    //     }
    struct rb_node *rb_right=node->rb_right;
    if (rb_right) {
        node = rb_right;
        struct rb_node *rb_left = node->rb_left;
        while (rb_left) {
            node=rb_left;
            rb_left = node->rb_left;
        }
        return (struct rb_node *)node;
    }
