...
typedef struct _xswork{
    int port;//牵引模式接收端口
    unsigned long long count;//记录每个端口接收的数量
    unsigned long long nParseCount;
    //unsigned long long
    sem_t  bin_sem;
    XS_LIST list;
    struct rb_root mapPartition;//qq/wechat/..otherlog
    struct rb_root mapPartitionUrl;//urllog
    struct rb_root mapPartitionDns;//dnslog
    struct rb_root mapPartitionID;//IDlog
    struct rb_root mapPartitionFileDw;//ntop数据表里的tbfiledwlog分区map
}XsWork,*PXsWork;
#define XSWORK_SIZE       sizeof(XsWork)
...
public :
    int g_signal;
    time_t g_xstime;
    int g_xsDebug;//全局打印信息
    u_long nFailedAnalysis;//
    //    MYDBOP g_xsMySql;
    HttpCMulti *httpCMulti;
    ES *elasticsearch;

    //镜像模式
    XS_LIST listUrl;//微信视频链接
    struct rb_root mapPartition;//qq/wechat/..otherlog
    struct rb_root mapPartitionUrl;//urllog
    struct rb_root mapPartitionFileDw;//ntop数据表里的tbfiledwlog分区map
    struct rb_root mapPartitionDhcp;//dhcp cache
    struct rb_root mapPartitionPicFile;//图片浏览和文件下载
    struct rb_root mapInterfaceName;//网卡信息(name--key)
    struct rb_root mapInterfaceID;//网卡信息(id--key)
    pIf_Msg pIfMsg;
    //    int port;//牵引模式接收端口
    //    sem_t  bin_sem;
    //    XS_LIST list;
    ...
    //    struct rb_root mapPartitionID;//IDlog
    //牵引模式
    struct rb_root mapXsWork;
    //verify
    struct key_tree g_machine_partition;
    char *g_path;
    u_int g_HostID;
    XSIDC g_xsIdc;
    char g_mac[20];
...
private:
    ...
    char epoch_buf[11];
    char working_dir[MAX_PATH]; /**< Array of working directory. *///"/var/tmp/ntopng
    char install_dir[MAX_PATH]; /**< Array of install directory. *///ntop/ntopng
    char startup_dir[MAX_PATH]; /**< Array of startup directory. *///root/xslogupdata/Build/Debug/bin
    char *custom_ndpi_protos; /**< Pointer of a custom protocol for nDPI. *///CML -p
    NetworkInterface *iface[MAX_NUM_DEFINED_INTERFACES];/**< Array of network interfaces. */
    u_int8_t num_defined_interfaces; /**< Number of defined interfaces. *///注册接口的数量
...
public:
   ...
   */
    ...
    /*~~~~~~byzpl~~~~~~~~*/
    void MyDBOp_Insert_Interface_Info();
   ...
   */
    inline NetworkInterface* getInterfaceAtId(int test,/*lua_State *vm, */int i) const {
        if(i<num_defined_interfaces && iface[i]) {
            //         return isInterfaceAllowed(vm, iface[i]->get_name()) ? iface[i] : NULL;//always return  true
            return iface[i];//
        }
        return NULL;
    }
   ...
   */
    inline NetworkInterface* getInterfaceAtId(int i) const {
        return getInterfaceAtId(0/*NULL*/, i);
    }
   ...
   */
    ...
    inline ES*                   getES()                      {return(elasticsearch);    };
