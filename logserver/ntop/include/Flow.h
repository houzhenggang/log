...
private:
    Host *cli_host, *srv_host;//...client ,server
    ...
    u_int16_t diff_num_http_requests;//...  count
#ifdef NTOPNG_PRO
    ...
    union {
        struct {
            char *last_url, *last_method;//...http url;http request type(get,put,...)
            char *last_content_type;
            char *last_host;//byzpl
            u_int16_t last_return_code;// http reply code(200...)
        } http;
    ...
    /*~~~~~~~~~~~~~byzpl~~~~~~~~*/
    u_int8_t flag;//insert mysql flag
    struct{
        char *id;
    }wechat;
    struct{
    ...
    struct {
        char *mailId;//sender   mail
        char *mailRcvId;//Receiver mail
    }e_mail;
    /*~~~~~~~~~~~~~~~~~~~~~*/
    struct {
...
public:
    ...
    /*~~byzpl~~~~*/
    inline bool isWECHAT()            {return(isProto(NDPI_PROTOCOL_WECHAT));}
    inline bool isQQ()                    {return(isProto(NDPI_PROTOCOL_OICQ));}
    inline bool isWEIBO()                    {return(isProto(NDPI_PROTOCOL_WEIBO));}
    /*~~~~~~~~~~~*/
    ...
    char* getFlowInfo(char *buffer,int size);

    /* *~~~~~~byzpl(only recode  url/qq/wechat/weibo flow )~~~~~~~* */
    int getFlowInfoToMysql(char *buffer, int size, char *type, char *act, char *mac, u_int date);
    ...
    /*~~~~~~~~byzpl(http_host)~~~~~~~~~~~*/
    inline char* getHTTPHOST()   {return(isHTTP()?protos.http.last_host:(char *)"");}//byzpl
    inline void  setHTTPHOST(char *v)  { if(isHTTP()) { if(protos.http.last_host) {free(protos.http.last_host); protos.http.last_host=NULL;}  protos.http.last_host = strdup(v); } }
    /*~~~byzpl(wechat)~~~*/
    inline char* getWECHATnum() {return (isWECHAT()?wechat.id:(char*)"");}
    inline  void   setWECHATnum(char *v){if(isWECHAT()){if(wechat.id)free(wechat.id);wechat.id=strdup(v);}}
    /*~~~~~~byzpl(oicq(qq))~~~~~~~~*/
    inline char * getQQnum() {return(isQQ()?qq.id:(char *)"");}
    inline  void   setQQnum(char *v){if(isQQ()){if(qq.id)free(qq.id);qq.id=strdup(v);}}
    /*~~~~~~byzpl(weibo)~~~~~~~~*/
    inline char * getWEIBOnum() {return(isWEIBO()?weibo.id:(char *)"");}
    inline  void   setWEIBOnum(char *v){if(isWEIBO()){if(weibo.id)free(weibo.id);weibo.id=strdup(v);}}
    /*~~~~~~~~~byzpl(get json object return backstage)~~~~~~~~~~~~*/
    void Ret_getJSONObject_Flow(json_object *pArray,u_int64_t numFlows);
