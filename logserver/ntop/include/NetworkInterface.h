...
protected:
    ...
    next_idle_flow_purge, next_idle_host_purge;//下一次删除时间
...
public:
  ...
  */
    ...
    inline int  get_id()                         { return(id);                                           };//interface ID
...
//    int dumpLocalHosts2redis(bool disable_purge);
   ...
   */
    ...
    /*~~~~~~~~~~~~byzpl~~~~~~~~~~~~~~~~~*/
    int getAllMacs(PXSJSONMAC json_mac,json_object *pRetRoot);
    int getAllFlows(PXSJSONFLOW json_flow,json_object *pRetRoot);
    int getAllHosts(PXSJSONHOST json_host,json_object *pRetRoot);
    int getAllInterfaceStats(json_object *pRetRoot);

    int getPartHosts(json_object *pRetRoot, PXSJSONHOST json_host);
    int getPartFlows(json_object *pRetRoot, PXSJSONFLOW json_flow);
    int  getPartMacs(PXSJSONMAC json_mac, json_object *pRetRoot);
    /*~byzpl(get json object return backstage)~~*/
    void Ret_getJSONObject_Interface(json_object *pArray);
    /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
