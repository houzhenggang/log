...
class Paginator {
protected:
    u_int16_t max_hits, to_skip;//max_hits界面一页展示的数据条目数10/20/50...,to_skip界面查询时起始数据条目数(点击下一页时将展示的第一条数据在当前hash的位置)
//   u_int16_t web_max_hits,web_to_skip;//byzpl web 界面一页展示
    bool a2z_sort_order;
...
//    virtual void readOptions(lua_State *L, int index);
    ...
    //byzpl  匹配后台界面参数
    void readWebOptions(PXSJSONFLOW json_flow);


    inline u_int16_t maxHits() const    { return(min_val(max_hits, CONST_MAX_NUM_HITS));  }
    inline u_int16_t toSkip() const     { return(to_skip);  }
//byzpl
//    inline u_int16_t webMaxHits() const    { return(min_val(web_max_hits, CONST_MAX_NUM_HITS));  }
//    inline u_int16_t webToSkip() const      { return(web_to_skip);  }
    ...
    inline int get_client_mode()  const { return(client_mode);};//byzpl
