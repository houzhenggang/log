...
#define MAX_NUM_INTERFACE_HOSTS   131072   //max_num_flows/max_num_hosts
...
#define FLOW_PURGE_FREQUENCY      1 /* sec */ //flow的删除频率 每秒清理一次
#define HOST_PURGE_FREQUENCY      1 /* sec */ //host的删除频率 每秒清理一次
#define MAX_TCP_FLOW_IDLE         3 /* sec - how long to delete a flow where the 3wh termination has been seen *///tcp flow每3秒清理一次(也就是说置于空闲时保证时间大于最后收包时间+3)
#define MAX_FLOW_IDLE            60 /* sec */// flow置于空闲的时间间隔 60S (界面配置参数可更改该值)
#define MAX_LOCAL_HOST_IDLE     300 /* sec */// local host置于空闲的时间间隔 300S (界面配置参数可更改该值)(mac/vlan/ases都使用的该值)
#define MAX_REMOTE_HOST_IDLE     60 /* sec *///remote host 置于空闲的时间间隔 60S (界面配置参数可更改该值)
...
#define HOST_SERIALIZED_KEY     "ntopng.serialized_hosts.ifid_%u_%s_%s@%d"   //...redis table name
...
#define CONST_ALT2_INSTALL_DIR         "/opt/xslog/xslogweb"    //test   /ntop/ntopng  "/usr/local/share/ntopng"
