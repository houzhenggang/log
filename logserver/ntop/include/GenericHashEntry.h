...
class GenericHashEntry {
 ...
 protected:
  u_int16_t num_uses;
  bool will_be_purged; /**< Mark this host as candidate for purging.*/ //标记是否清除
 ...
 public:
  ...
  virtual bool idle();
  virtual void set_to_purge()          { will_be_purged = true; };//设置为删除标志
  inline bool is_ready_to_be_purged() { return(will_be_purged); };//设置为删除标志
