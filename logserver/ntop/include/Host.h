...
private:
    ...
    AutonomousSystem *as;
    Vlan *vlan;
    char *symbolic_name, *continent, *country, *city, *asname, os[32], trafficCategory[12], *info;
...
public:
    ...
    char* getJSON();
    inline void setOS(char *_os)
    {
//        memset(os,0,sizeof(os));
        if(memcmp(os,_os,sizeof(os))){
              snprintf(os, sizeof(os)-1, "%s", _os);
        }
//        else
//            fprintf(stdout,"Err\n");
...
//    void refreshHostAlertPrefs();
    ...
    void Ret_getJSONObject_Host(json_object *pArray,u_int64_t numHosts);
