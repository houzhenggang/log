...
class Prefs {
 private:
  ...
  bool enable_dns_resolution, sniff_dns_responses,
    ...
    enable_auto_logout/*logout detele*/, use_promiscuous_mode, slack_notifications_enabled,
  ...
  u_int http_port/*3000*/, alt_http_port, https_port;
  ...
  char *packet_filter;
  char *user;
  char *redis_host;//default 127.0.0.1  CMI -r
  char *redis_password;//default NULL   CMI -r
  char *pid_path;
  char *cpu_affinity;
  u_int32_t safe_search_dns_ip, global_primary_dns_ip, global_secondary_dns_ip;
  u_int8_t redis_db_id;// default 0 CMI -r
  int redis_port;//default 6379 CMI -r
  int dns_mode;
  ...
  inline void nDPIhelp()  { nDPIusage(); }//nDPIhelp()  print-ndpi-protocols
...
//  void setTraceLevelFromRedis();
 ...
 public:
  ...
  u_int8_t daemon_mode;//daemon running
  ...
  inline char* get_docs_dir()                           { return(docs_dir);       }; // HTTP docs(后台)
  inline char* get_scripts_dir()                        { return(scripts_dir);    };//lua scripts (后台)
  ...
  inline bool  do_auto_logout()                         { return(enable_auto_logout);               };/*logout detele*/
