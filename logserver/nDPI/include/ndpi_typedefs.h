...
struct ndpi_packet_struct {
    ...
    struct ndpi_int_one_line_struct http_url_name;//http_path+http_param
    ...
    struct ndpi_int_one_line_struct http_log_uid;//byzpl
    struct ndpi_int_one_line_struct http_param;//byzpl
    struct ndpi_int_one_line_struct http_path;//byzpl
...
typedef struct ndpi_proto_defaults {
    ...
    u_int16_t protoId/*tcp/udp ~*/, protoIdx;
...
typedef enum  {
    ...
    virtual_weixin,
    virtual_qq,
    virtual_weibo, //sina weibo
    virtual_mail,    //e-mail
    virtual_wangwang  //aliwangwang
}virtual_type;
//statu
typedef enum{
    vrt_method_unknow=0,
...
typedef struct{
    ...
    mail_proType proType;
    qq_wechat_msg TypeMsg;
    wangwang_version  wangVer;
    u_int64_t id;//qq/wechat/weibo
    u_int64_t hashkey;//file name
    char szUrl[1500];//dwfile url
    char szNum[32];//mail_Sender/wangwang
    char mail_rcv[32];//mail_Receiver
    char vedioUrl[1500];//tcp wechat vedio url
}VirtualNum;
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
/*这个结构体用于存储一个数据流的相关信息，一个数据流可能会有很多个数据包。
*正因如此，所以在此结构体中定义了很多标识变量（有初始赋值的），用于区别不
* 同的数据包和减少重复多余的工作。此外，还针对不同协议定义了很多变量。
*/
...
struct ndpi_flow_struct {
    ...
    u_int16_t guessed_protocol_id/*tcp/udp/icmp...~three layer*/, guessed_host_protocol_id;
