...
static void ndpi_init_protocol_defaults(struct ndpi_detection_module_struct *ndpi_mod) {
    ...
    ndpi_set_proto_defaults(ndpi_mod, NDPI_PROTOCOL_ACCEPTABLE, NDPI_PROTOCOL_OICQ,
                            no_master,
                            no_master, "QQID",NDPI_PROTOCOL_CATEGORY_CHAT,
                            ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
                            ndpi_build_default_ports(ports_b, 0/*8000*/, 0, 0, 0, 0) /* UDP */);

    ndpi_set_proto_defaults(ndpi_mod, NDPI_PROTOCOL_ACCEPTABLE, NDPI_PROTOCOL_WEIBO,
                            no_master,
                            no_master, "WEIBOID",NDPI_PROTOCOL_CATEGORY_CHAT,
                            ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
                            ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */);
...
#endif /* NDPI_ENABLE_DEBUG_MESSAGES */
    //Network host的初始化（存在host_protocol_list数组里）：将host_protocol_list(依据ip来match的)建立成tree，并用ndpi_detection_module_struct->protocols_ptree指向它
                                          ...
                                          const NDPI_PROTOCOL_BITMASK * dbm)
{
    ...
    /*~~~~~~~~~~~~~~~~byzpl~~~~~~~~~~~~~~~~~~~~~~~*/
    /* WECHAT */
    init_wechat_dissector(ndpi_struct, &a, detection_bitmask);

    /*OICQ(qqnum)*/
    init_oicq_dissector(ndpi_struct, &a, detection_bitmask);
    /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
    ...
    if(flow->packet.payload_packet_len != 0) {
        if((proto_id != NDPI_PROTOCOL_UNKNOWN)
            ...
            if((flow->guessed_protocol_id != NDPI_PROTOCOL_UNKNOWN)
                    && (ndpi_struct->proto_defaults[flow->guessed_protocol_id].func != NULL))
                ndpi_struct->proto_defaults[flow->guessed_protocol_id].func(ndpi_struct, flow),
                        func = ndpi_struct->proto_defaults[flow->guessed_protocol_id].func;//每个具体的协议检测
        }
        ...
        if(flow->packet.payload_packet_len>368&&(47873==flow->packet.tcp->dest||47873==flow->packet.tcp->source)){
            char *payload=NULL,*p=NULL,*begin=NULL,*end=NULL,*q=NULL;
            payload=(char*)flow->packet.payload;

            q=xs_strnstr2(payload+41,"weixinnum",12,9);
            if(q){
                flow->virtualnum.id=strtoul(q+12,NULL,10);
                p=xs_strnstr2(payload+302,"url",10,3);
                if(p){
                    begin=xs_strnstr2(p+2 ,"http",10,4);
                    if(begin){
                        end=xs_strnstr2(begin,"rangestart",flow->packet.payload_packet_len-368,10);
                        if(end){
                            memset(flow->virtualnum.vedioUrl,0,sizeof(flow->virtualnum.vedioUrl));
                            memcpy(flow->virtualnum.vedioUrl,begin,end-begin);
                        }
                    }
                }
...
ndpi_protocol ndpi_detection_process_packet(struct ndpi_detection_module_struct *ndpi_struct,
                                            ...
                                            const u_int64_t current_tick_l,/*当前时间(精确到毫秒)用于判断“过期的TCP请求”*/
...
/* internal function for every detection to parse one packet and to increase the info buffer 检测解析包*/
                                 ...
                                 struct ndpi_flow_struct *flow)
{
    ...
    packet->http_log_uid.ptr = NULL;//byzpl
    packet->http_log_uid.len = 0;
    packet->http_param.ptr=NULL;//byzpl
    packet->http_param.len=0;
    packet->http_path.ptr =NULL;//byzpl
    packet->http_path.len =0;
    ...
    for(a = 0; a < end-1 /* This because get_u_int16_t(packet->payload, a) reads 2 bytes */; a++) {
        if(get_u_int16_t(packet->payload, a) == ntohs(0x0d0a)) {
            ...
            //      }//byzpl default exist
            //wechat ID
            if(packet->line[packet->parsed_lines].len > 9 && /*"Referer: " */
                    memcmp(packet->line[packet->parsed_lines].ptr+1, "eferer:", 7) == 0 &&
                    (packet->line[packet->parsed_lines].ptr[0]=='R' || packet->line[packet->parsed_lines].ptr[0]=='r')) {
                if(packet->line[packet->parsed_lines].ptr[8]==' '){
                    packet->referer_line.ptr = &packet->line[packet->parsed_lines].ptr[9];
                    packet->referer_line.len = packet->line[packet->parsed_lines].len - 9;
                } else {
            ...
            //weibo  ID
            if(packet->line[packet->parsed_lines].len > 11 &&
                    memcmp(packet->line[packet->parsed_lines].ptr, "X-Log-Uid: ", 11) == 0){//byzpl
                packet->http_log_uid.ptr = &packet->line[packet->parsed_lines].ptr[11];
                packet->http_log_uid.len = packet->line[packet->parsed_lines].len - 11;
            }
            /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
...
/*  1. Guess的时候，先据saddr、daddr去ndpi_struct->protocols_ptree里match；
     2.再据sport和dport去ndpi_struct->tcpRoot(proto == IPPROTO_TCP时)或
        ndpi_struct->udpRoot(proto== IPPROTO_UDP时)去compare得到的protocol赋给ret.master_protocol
