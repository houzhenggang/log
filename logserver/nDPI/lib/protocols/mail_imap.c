...
void ndpi_search_mail_imap_tcp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
    ...
    /*~~~~~~~~~~~~~~byzpl~~~~~~~~~~~~~~*/
    if((IMAP_HTONS143==packet->tcp->dest||IMAP_HTONS143==packet->tcp->source)&&packet->payload_packet_len>7){
        if ((packet->payload[3] == 'L' || packet->payload[3] == 'l')//登录邮箱
                && (packet->payload[4] == 'O' || packet->payload[4] == 'o')
                && (packet->payload[5] == 'G' || packet->payload[5] == 'g')
                && (packet->payload[6] == 'I' || packet->payload[6] == 'i')
                && (packet->payload[7] == 'N' || packet->payload[7] == 'n')) {

            char *p,*q;
            char * payload=(char *)packet->payload;
            p=(char *)malloc(2048);
            memset(p,0,sizeof(p));
            memcpy(p,payload+9,packet->payload_packet_len);
            q=strstr(p," ");

            memcpy(flow->virtualnum.szNum,p,q-p);
            flow->virtualnum.type=virtual_mail;
            flow->virtualnum.status=vrt_method_online;
            flow->virtualnum.proType=mail_imap;
            free(p);
        }
    }
        /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
        ...
        if (packet->payload_packet_len >= 4 && ntohs(get_u_int16_t(packet->payload, packet->payload_packet_len - 2)) == 0x0d0a) {
            ...
            if (packet->payload_packet_len == 6 && ((packet->payload[0] == 'D' || packet->payload[0] == 'd')
                ...
                if ((command_start + 10) < packet->payload_packet_len) {//返回IMAP服务器支持的功能列表
                ...
                if ((command_start + 5) < packet->payload_packet_len) {
                    if ((packet->payload[command_start] == 'L' || packet->payload[command_start] == 'l')//登录邮箱
                        ...
                        flow->l4.tcp.mail_imap_stage += 1;
                        saw_command = 1;
                    } else if ((packet->payload[command_start] == 'F' || packet->payload[command_start] == 'f')//得到所有邮件的UID
                        ...
                        flow->l4.tcp.mail_imap_stage += 1;
                        saw_command = 1;
                    } else if ((packet->payload[command_start] == 'C' || packet->payload[command_start] == 'c')//用来在邮箱中设置一个检查点
                        ...
                        flow->l4.tcp.mail_imap_stage += 1;
                        saw_command = 1;
                    } else if ((packet->payload[command_start] == 'S' || packet->payload[command_start] == 's')//用于修改指定邮件的属性，包括邮件打上已读标记，删除标记等
                               && (packet->payload[command_start + 1] == 'T' || packet->payload[command_start + 1] == 't')
                ...
                if ((command_start + 4) < packet->payload_packet_len) {//列出活动的文件夹
                    if ((packet->payload[command_start] == 'L' || packet->payload[command_start] == 'l')
                        ...
                        flow->l4.tcp.mail_imap_stage += 1;
                        saw_command = 1;
                    } else if ((packet->payload[command_start] == 'L' || packet->payload[command_start] == 'l')//列出邮箱所有文件夹
                               && (packet->payload[command_start + 1] == 'I' || packet->payload[command_start + 1] == 'i')
                        ...
                        flow->l4.tcp.mail_imap_stage += 1;
                        saw_command = 1;
                    } else if ((packet->payload[command_start] == 'N' || packet->payload[command_start] == 'n')//空语句，用于保持连接
                               && (packet->payload[command_start + 1] == 'O' || packet->payload[command_start + 1] == 'o')
                               && (packet->payload[command_start + 2] == 'O' || packet->payload[command_start + 2] == 'o')
                ...
                if ((command_start + 6) < packet->payload_packet_len) {//进入收件箱
                    if ((packet->payload[command_start] == 'S' || packet->payload[command_start] == 's')
                            && (packet->payload[command_start + 1] == 'E' || packet->payload[command_start + 1] == 'e')
                            && (packet->payload[command_start + 2] == 'L' || packet->payload[command_start + 2] == 'l')
                            && (packet->payload[command_start + 3] == 'E' || packet->payload[command_start + 3] == 'e')
...
imap_excluded:
        ...
        if ((packet->payload_packet_len >= 2 && ntohs(get_u_int16_t(packet->payload, packet->payload_packet_len - 2)) == 0x0d0a)
                && flow->packet_counter <=6 && flow->l4.tcp.mail_imap_stage >= 1) {//test
            NDPI_LOG(NDPI_PROTOCOL_MAIL_IMAP, ndpi_struct, NDPI_LOG_DEBUG,
